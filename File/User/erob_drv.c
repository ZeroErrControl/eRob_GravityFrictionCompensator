...
void EROB_DRV_init(void)
{
    ...
    h->k_radps2simps = 1.0f / YZDN_MATH_2PI * h->encoder_sim;
    ...
    switch(h->state)
    {
        ...
        case EROB_DRV_STATE_TX_CMD: // 设置电流目标值
        {
            ...
            else if(h->mode == EROB_DRV_MODE_SPD)
            {
                ...
                h->speed_ref_rpm = h->speed_ref * YZDN_MATH_K_RADPS2RPM;
        ...
        case EROB_DRV_STATE_RX_SPD_ACK: // 解析转速反馈
        {
            if(h->can_rx_flag)
            {
                ...
                if((h->can_rx_len == 0x05)&&(h->can_rx_data[4]== 0x3e))
                {
                    ...
                    h->speed = h->speed_raw * h->encoder_sim_div * YZDN_MATH_2PI;
                    h->speed_rpm = h->speed * YZDN_MATH_K_RADPS2RPM;
        ...
        case EROB_DRV_STATE_RX_POS_ACK: // 解析位置反馈
        {
            if(h->can_rx_flag)
            {
                ...
                if((h->can_rx_len == 0x05)&&(h->can_rx_data[4]== 0x3e))
                {
                    ...
                    h->angle = h->angle_raw * h->encoder_sim_div * YZDN_MATH_2PI;
                    h->angle_deg = h->angle * YZDN_MATH_K_RAD2DEG;
        ...
        case EROB_DRV_STATE_RX_TORQUE_ACK: // 解析扭矩数据
        {
            if(h->can_rx_flag)
            {
                ...
                if((h->can_rx_len == 0x05)&&(h->can_rx_data[4]== 0x3e))
                {
                    ...
                    h->angle_delta = h->torque_raw * h->encoder_sim_div * YZDN_MATH_2PI;
                    h->angle_delta_deg = h->angle_delta * YZDN_MATH_K_RAD2DEG;
                    h->state = EROB_DRV_STATE_TX_EN;
                }
                else
                {
